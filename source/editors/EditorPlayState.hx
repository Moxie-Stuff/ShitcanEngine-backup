...
class EditorPlayState extends MusicBeatState
{
	...
	private function generateStaticArrows(player:Int, ?skip:Bool = false):Void
	{ // TODO: remove this and just add the PlayField where neccessary
		var x:Float = FlxG.width / 2 + (FlxG.width / 4);
		if (player == 1)
			x = FlxG.width / 2 - (FlxG.width / 4);
		
		var field:PlayField = new PlayField(x, strumLine.y, 4, null, player == 1, player == 0);

		if (player == 1)
			playerStrums = field;
		else
			opponentStrums = field;
		...
		/*
					playerField = new Playfield(FlxG.width/2 + (FlxG.width/4), 50, style, boyfriend, true, this);
			dadField = new Playfield(FlxG.width/2 - (FlxG.width/4), 50, style, dad, false, this);
		 */
	}
